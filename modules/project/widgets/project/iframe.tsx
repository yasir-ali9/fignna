...
import { IframeCommunication, createParentCommunication } from '@/lib/iframe/communication';
import type { DomElement, IframeChildMethods, IframeParentMethods } from '@/lib/iframe/penpal-types';
...
export const Iframe = observer(({ 
  ...
  const setupCommunication = useCallback(() => {
    ...
    comm.registerMethods(parentMethods);
    ...
    setTimeout(async () => {
      try {
        ...
        console.log('Communication not available (expected for external sites):', error);
      }
    }, 1000);

  }, [enableCommunication, frameId, onElementSelected, onElementHovered, onDomChanged]);
  ...
  const communicationMethods = {
    async getElementAtLoc(x: number, y: number): Promise<DomElement | null> {
      ...
      try {
        return await communicationRef.current.callMethod('getElementAtLoc', x, y);
      } catch (error) {
    ...
    async highlightElement(selector: string): Promise<void> {
      ...
      try {
        await communicationRef.current.callMethod('highlightElement', selector);
      } catch (error) {
        console.error('Failed to highlight element:', error);
      }
    },

    async updateElementStyle(selector: string, styles: Record<string, string>): Promise<boolean> {
      if (!communicationRef.current || !isCommReady) return false;
      try {
        return await communicationRef.current.callMethod('updateStyle', selector, styles);
      } catch (error) {
        ...
        {isLoading ? (
          ...
          <div className="flex items-center justify-center bg-bk-60 h-full">
            <div className="text-center text-fg-40">
              ...
              <div className="text-xs text-fg-30 mt-1">Create a project to get started</div>
            </div>
          </div>
        )}
      </div>
  ...
  if (hasError) {
    return (
      <div className={`flex items-center justify-center bg-bk-50 ${className}`}>
    ...
    <div className={`relative ${className}`}>
      ...
      {enableCommunication && !iframeLoading && (
        <div className="absolute top-2 right-2 z-20">
          <div className={`w-2 h-2 rounded-full ${isCommReady ? 'bg-green-400' : 'bg-gray-400'}`} 
               title={isCommReady ? 'Communication ready' : 'Communication not available'} />
        </div>
      )}
